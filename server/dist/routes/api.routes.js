...
router.post('/upload', authMiddleware, requireRole('ADMIN', 'MANAGER'), upload.single('file'), (req, res) => {
...
router.patch('/users/:id/password', authMiddleware, requireRole('ADMIN'), async (req, res) => {
    const id = req.params.id;
    if (!id)
        return res.status(400).json({ error: 'User ID is required' });
...
router.post('/documents/:id/sync', authMiddleware, async (req, res) => {
    const { id } = req.params;
    if (req.user.role !== 'ADMIN')
        return res.status(403).json({ error: 'Admin only' });
    try {
