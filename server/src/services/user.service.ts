...
export class UserService {
  ...
  async create(userData: CreateUserDTO): Promise<User | null> {
    ...
    if (this.isLocalMode) {
      ...
      const existing = this.sqlite.getDatabase()
        .prepare('SELECT id, email, name, role, department, status FROM users WHERE email = ?')
        .get(normalizedEmail);
      if (existing) {
        return null;
      }
      ...
      this.sqlite.getDatabase()
        .prepare('INSERT INTO users (id, email, name, password_hash, role, department, status) VALUES (?, ?, ?, ?, ?, ?, ?)')
        .run(id, normalizedEmail, userData.name.trim(), password_hash, userData.role || 'VIEWER', userData.department || 'General', 'Active');
      return this.getById(id);
    } else if (this.supabase) {
      const { data, error } = await this.supabase
        .from('users')
