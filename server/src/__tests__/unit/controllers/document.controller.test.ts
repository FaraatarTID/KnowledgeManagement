...
describe('DocumentController', () => {
    ...
    describe('upload', () => {
        ...
        it('should index local PDF content when drive is not configured', async () => {
            ...
            const indexCallArgs = vi.mocked(syncService.indexFile).mock.calls[0];
            expect(indexCallArgs[0]).toEqual(expect.objectContaining({
                id: expect.stringMatching(/^manual-/)
            }));
            expect(indexCallArgs[1]).toBeUndefined();
            expect(indexCallArgs[2]).toEqual({
                localFilePath: tempUploadPath
            });
            expect(indexCallArgs[2]?.localFileBuffer).toBeUndefined();
            expect(historyService.recordEvent).toHaveBeenCalledWith(expect.objectContaining({
